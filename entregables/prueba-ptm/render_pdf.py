...
def render_slide(html_path: Path, index: int, out_png: Path) -> None:
    if out_png.exists():
        out_png.unlink()
...
"""
    ...
    cmd = [
        ...
        "--virtual-time-budget=3500", f"file://{temp}",
    ]
...
def crop_exact(png: Path) -> pymupdf.Pixmap:
    ...
    return pymupdf.Pixmap(pix, clip)
...
def build_pdf(html_path: Path, n_slides: int, out_pdf: Path) -> None:
    ...
    for i in range(n_slides):
        ...
        jpg.write_bytes(pix.tobytes("jpeg", jpg_quality=90))
        jpgs.append(jpg)
